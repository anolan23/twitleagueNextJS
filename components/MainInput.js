...
function MainInput(props) {
    ...
    const chars = () => contentEditable.current ? contentEditable.current.innerText.length : 0;
    ...
    const onSubmit = (event) => {
        ...
        if(files){
            ...
            Promise.all(promises).then((uploadedFiles) => onUploadToS3(uploadedFiles))
    ...
    const renderMedia = () => {
        if(media === null){
            ...
            return media.map((mediaItem, index) => {
                ...
                return (
                        <TwitMedia key={index} close media={mediaItemArray} onClick={onTwitMediaClose}/> 
                ) 
            })
        }     
    }
    ...
    const handleUploadChange = event => {
        ...
        if(numVideos === 1 && numImages === 0 && numGifs === 0){
            ...
            files.map(file => promise.then(() => promiseFileReader(file)).then((dataUrl) => {
        ...
        <form className={props.compose ? `${mainInput["main-input"]} ${mainInput["main-input__compose"]}` : mainInput["main-input"]} onSubmit={onSubmit} onKeyDown={handleKeyDown}>
            ...
            <div className={mainInput["main-input__actions"]}>
                <div className={mainInput["main-input__media-types"]}>
                        <TwitIcon onClick={onUploadClick} className={mainInput["main-input__media-types__icon"]} icon="/sprites.svg#icon-image"></TwitIcon>
                        <input multiple type="file" ref={hiddenFileInput} onChange={handleUploadChange} style={{display:"none"}}></input>
                        <TwitIcon onClick={props.toggleGifPopup} className={mainInput["main-input__media-types__icon"]} icon="/sprites.svg#icon-plus-circle">GIF</TwitIcon>
                        <TwitIcon onClick={onMoneyClick} className={mainInput["main-input__media-types__icon"]} icon="/sprites.svg#icon-map-pin">$</TwitIcon>
                        <TwitIcon onClick={onAtClick} className={mainInput["main-input__media-types__icon"]} icon="/sprites.svg#icon-bookmark">@</TwitIcon>

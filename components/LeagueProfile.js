...
function LeagueProfile(props) {
  ...
  const renderButton = () => {
    ...
    if(user.username === league.owner){
      ...
      return <TwitButton color="twit-button--primary" outline="twit-button--primary--outline">Unfollow</TwitButton>
    ...
    <React.Fragment>
      ...
      >
        <div className={leagueProfile["league-profile__info"]}>
            <div className={`${leagueProfile["league-profile__teamname-box"]} u-margin-top-tiny`}>
              <h1 className="heading-1">{league.league_name}</h1>
            </div>
            {league.bio ? <p className={leagueProfile["league-profile__info__bio"] + " muted"}>{league.bio}</p> : null}
            <div className={leagueProfile["league-profile__info__name"]}>
              <span className={leagueProfile["league-profile__info__name__league"]}>Owner: </span>
              &nbsp;
              {renderOwner()}
            </div>
            <div className={leagueProfile["league-profile__attributes"]}>
              <Attribute icon={"/sprites.svg#icon-map-pin"} text={league.sport}/>
              <Attribute icon={"/sprites.svg#icon-home"} text={`Joined ${league.joined}`}/>
            </div>
